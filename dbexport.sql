...
CREATE TABLE `actualtripstopinfo` (
  ...
  `SecheduledArrivalTime` time DEFAULT NULL,
  ...
  CONSTRAINT `actualtripstopinfo_ibfk_1` FOREIGN KEY (`TripNumber`) REFERENCES `trip` (`TripNumber`)
...
CREATE TABLE `driver` (
  ...
  `DriverTelephoneNumber` varchar(10) DEFAULT NULL,
...
CREATE TABLE `tripoffering` (
  ...
  KEY `BusID` (`BusID`),
  CONSTRAINT `tripoffering_ibfk_1` FOREIGN KEY (`DriverName`) REFERENCES `driver` (`DriverName`),
  CONSTRAINT `tripoffering_ibfk_2` FOREIGN KEY (`BusID`) REFERENCES `bus` (`BusID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2021-05-13 16:21:11
